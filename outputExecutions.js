...
const executionsQuery = '' + fs.readFileSync('./queries/executions.rq');
...
async function calculateStatsForDataset(dataset, datasetId, options) {
    ...
    const result = await comunicaEngine.query(
        executionsQuery.replaceAll('$dataset', `<${dataset}>`), {
        sources: [{
            type: 'sparql',
            value: inputEndpointURL
        }],
        initialBindings: bindingsFactory.fromRecord({
            dataset: rdfFactory.namedNode(dataset)
        })
...
async function main() {
    calculateStats({
        ...
        outputDirPath: './input/executions/csv/',
        excludeDatasets: [
            ...
            // 'http://lsq.aksw.org/datasets/bench-wikidata-interval1-organic-lsq2', // missing
